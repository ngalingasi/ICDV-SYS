...
class _SplashScreenState extends ConsumerState<SplashScreen>
  ...
  void initState() {
    ...
    _fade     = CurvedAnimation(parent: _ctrl, curve: const Interval(0.0, 0.45, curve: Curves.easeOut));
    _scale    = Tween<double>(begin: 0.75, end: 1.0).animate(
        CurvedAnimation(parent: _ctrl, curve: const Interval(0.0, 0.45, curve: Curves.easeOutBack)));
    _progress = CurvedAnimation(parent: _ctrl, curve: const Interval(0.35, 0.95, curve: Curves.easeInOut));
    _ctrl.forward().then((_) => Future.delayed(const Duration(milliseconds: 300), _navigate));
  }
            ...
            child: Transform.scale(scale: _scale.value,
              child: Column(mainAxisSize: MainAxisSize.min, children: [
                ...
                Text('CORPORATION SOLE', style: TextStyle(
                  color: c.textSecond, fontSize: 11,
                  fontWeight: FontWeight.w600, letterSpacing: 3)),
                const SizedBox(height: 6),
                Container(
                  padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 4),
                  decoration: BoxDecoration(
                    color: AppColors.navy.withOpacity(dark ? 0.5 : 0.08),
                    borderRadius: BorderRadius.circular(4),
                    border: Border.all(color: AppColors.navyLight.withOpacity(dark ? 0.6 : 0.2)),
                  ),
                  child: Text('TPFCS VEHICLE OPERATIONS', style: TextStyle(
                ...
                Text('Initialising secure session…', style: TextStyle(
                  color: c.textMuted, fontSize: 11, letterSpacing: 0.5)),
              ]),
            ),
          ),
        )),
