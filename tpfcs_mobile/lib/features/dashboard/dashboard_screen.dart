...
class DashboardStats {
  ...
  const DashboardStats({
    this.totalVessels = 0,
    this.totalManifests = 0,
    this.totalVehicles = 0,
    this.manifestedCount = 0,
    this.dischargedCount = 0,
    this.batchedCount = 0,
    this.inTransitCount = 0,
    this.receivedCount = 0,
    this.openBatches = 0,
    this.releasedVehicles = 0,
    this.unreleasedVehicles = 0,
  });
  factory DashboardStats.fromJson(Map<String, dynamic> j) {
    // ── Global dashboard: GET /dashboard ──────────────────────────────────────
    // Response: { stats: { total_vehicles, manifested_count, ... }, ... }
    //
    // ── Manifest dashboard: GET /dashboard/manifest/:id ───────────────────────
    // Response: { total_vehicles, workflow: { manifested, discharged, ... }, ... }
    // workflow is a Map<status, count> with plain status names (no _count suffix)

    final isManifestScope = j.containsKey('workflow') && j['workflow'] is Map;
...
class _DashboardScreenState extends ConsumerState<DashboardScreen> {
  ManifestSummary? _selectedManifest;
  ...
  Widget build(BuildContext context) {
    ...
    final initial =
        user?.fullName.isNotEmpty == true
            ...
            : 'U';
    final manifestAsync = ref.watch(manifestListProvider);
    final dash = ref.watch(dashboardProvider(_selectedManifest?.manifestId));
              ...
              SliverPadding(
                ...
                sliver: dash.when(
                  ...
                  data: (data) {
                    ...
                    final vessels =
                        (data['recent_vessels'] as List? ?? [])
                            .map(
                              (v) => RecentVessel.fromJson(
      ...
      decoration: BoxDecoration(
        gradient: LinearGradient(
          colors:
              dark
                  ? [
                    const Color(0xFFCFDEF7).withOpacity(0.22),
                    const Color(0xFFADC6F0).withOpacity(0.10),
                  ]
        ...
        border:
            dark
                ? Border(
                  bottom: BorderSide(
                    color: const Color(0xFF6AAEF5).withOpacity(0.25),
                  ),
                )
                : null,
      ),
  ...
  Widget build(BuildContext context) {
    final items = [
      _StatItem(
        ...
        const Color(0xFF0D4E9E),
        const Color(0xFFCFDEF7),
      ),
      _StatItem(
        'Manifests',
      ...
      _StatItem(
        'Released',
        stats.releasedVehicles,
        Icons.check_circle_outline,
        AppBrand.success,
        const Color(0xFFCCF0DC),
      ),
      _StatItem(
        'Open Batches',
        stats.openBatches,
                                ...
                                FractionallySizedBox(
                                  ...
                                  child: Container(
                                    ...
                                    decoration: BoxDecoration(
                                      ...
                                      boxShadow: [
                                        BoxShadow(
                                          color: step.dot.withOpacity(0.4),
                                          blurRadius: 4,
                                        ),
                                      ],
                                    ),
...
class _StepChip extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    // Map step key (e.g. 'manifested_count') to workflow_status value
    final statusKey = step.key.replaceAll('_count', '');
...
class _OperationsGrid extends StatelessWidget {
  ...
  static final _ops = [
    _OpItem(
      ...
      '/discharge',
      const Color(0xFFCFDEF7),
      const Color(0xFFADC6F0),
      const Color(0xFF6AAEF5),
      const Color(0xFF0D4E9E),
    ),
    _OpItem(
      'Batch',
    ...
    _OpItem(
      ...
      '/transfer',
      const Color(0xFFFDE8CC),
      const Color(0xFFF9D0A0),
      const Color(0xFFF5A652),
      AppBrand.orange,
    ),
    _OpItem(
      'Yard Receive',
      ...
      '/receive',
      const Color(0xFFCCF0DC),
      const Color(0xFFA0E0BC),
      const Color(0xFF4DC98A),
      AppBrand.success,
    ),
  ];
  ...
  Widget build(BuildContext context) => GestureDetector(
    ...
    child: Container(
      decoration: BoxDecoration(
        ...
        border:
            dark
                ? Border.all(color: op.symbol.withOpacity(0.35), width: 1.5)
                : null,
      ),
      child: ClipRRect(
        borderRadius: BorderRadius.circular(20),
...
class _RecentVessels extends StatelessWidget {
  final List<RecentVessel> vessels;
  final AppColors c;
  final bool dark;
  const _RecentVessels({
                      ...
                      vessels.asMap().entries.map((e) {
                        ...
                        return Column(
                          children: [
                            ...
                            if (!isLast)
                              Divider(height: 1, indent: 70, color: c.border),
                          ],
                        );
                      }).toList(),
                ),
