...
class _ReceiveScreenState extends ConsumerState<ReceiveScreen> {
  ...
  Future<void> _lookup() async {
    ...
    if (q.isEmpty) {
      setState(() => _error = 'Enter driver ID card number');
      return;
    }
    setState(() {
      _loading = true;
  ...
  Future<void> _showConfirmSheet() async {
    ...
    final ok = await showOpConfirmSheet(
      ...
      subtitle:
          'Mark ${_data!.vehicle.chassisNumber} as RECEIVED at ICDV Yard\nand close driver assignment?',
      ...
      successMessage:
          '${_data!.vehicle.chassisNumber} is now at ICDV Yard.\nDriver assignment closed.',
      onConfirm:
          () => ref
              .read(workflowApiProvider)
              .receiveConfirm(
                _data!.driver.driverId,
                _data!.vehicle.vehicleId,
  ...
  Widget _buildForm(AppColors c) => Column(
    ...
    children: [
      if (_step == _Step.lookup) ...[
        ...
        Text(
          'Scan or enter the driver\'s ID card to fetch their assigned vehicle.',
      ...
      if (_data != null) ...[
        ...
        if (_data!.transferredAt != null) ...[
          ...
          Text(
            'Transferred: ${_data!.transferredAt}',
