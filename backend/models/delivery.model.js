...
const createDelivery = async (body, creatorId, icdvId) => {
  const {
  ...
  const [vehicle] = await query(
    "SELECT vehicle_id, icdv_id, operational_status FROM vehicles WHERE vehicle_id=?", [vehicle_id]
  );
  ...
  const r = await query(
    `INSERT INTO deliveries (icdv_id, vehicle_id, driver_id, scheduled_date, delivery_address,
      recipient_name, recipient_phone, notes, status, created_by)
     VALUES (?,?,?,?,?,?,?,?,?,?)`,
    [icdvId, vehicle_id, driver_id, scheduled_date, delivery_address,
     recipient_name, recipient_phone, notes, status, creatorId]
  );
  return getDeliveryById(r.insertId, icdvId);
};

const getDeliveries = async ({ page, limit, vehicle_id, driver_id, status, search }, icdvId = null) => {
  const { limit: l, offset, paginate } = buildPagination(page, limit);
  let where = '1=1'; const params = [];
  if (icdvId !== null) { where += ' AND dl.icdv_id=?'; params.push(icdvId); }
  ...
  const deliveries = await query(
    `SELECT dl.*,
     ...
     WHERE ${where} ORDER BY dl.scheduled_date DESC, dl.created_at DESC LIMIT ? OFFSET ?`,
    [...params, l, offset]
  );
  return paginate(deliveries, total);
};

const getDeliveryById = async (id, icdvId = null) => {
  ...
  if (icdvId !== null && delivery.icdv_id !== icdvId)
    throw new ApiError(httpStatus.NOT_FOUND, 'Delivery not found');
  return delivery;
};

const updateDelivery = async (id, body, updaterId, icdvId = null) => {
  await getDeliveryById(id, icdvId);
  ...
  if (!fields.length) return getDeliveryById(id, icdvId);
  ...
  return getDeliveryById(id, icdvId);
};

const updateDeliveryStatus = async (id, newStatus, userId, data = {}, icdvId = null) => {
  const delivery = await getDeliveryById(id, icdvId);
  const allowed = DELIVERY_STATUS_TRANSITIONS[delivery.status] || [];
  if (!allowed.includes(newStatus))
    throw new ApiError(httpStatus.BAD_REQUEST, `Cannot transition from '${delivery.status}' to '${newStatus}'`);

  const extra = []; const extraP = [];
  ...
  await query(`UPDATE deliveries SET ${setClause} WHERE delivery_id=?`,
...
const deleteDelivery = async (id, icdvId = null) => {
  const delivery = await getDeliveryById(id, icdvId);
