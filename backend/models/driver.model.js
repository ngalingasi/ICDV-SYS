...
const createDriver = async (body, creatorId) => {
  const {
  ...
  const [r] = await query(
    `INSERT INTO drivers (full_name, license_number, phone, email, id_number, status, notes, created_by)
     VALUES (?,?,?,?,?,?,?,?)`,
    [full_name, license_number, phone, email, id_number, status, notes, creatorId]
  );
...
const getDrivers = async ({ page, limit, status, search }) => {
  ...
  if (search) {
    where += ' AND (d.full_name LIKE ? OR d.license_number LIKE ? OR d.phone LIKE ?)';
    const s = `%${search}%`;
    params.push(s, s, s);
  }
...
const updateDriver = async (id, body, updaterId) => {
