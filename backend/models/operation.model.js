...
const createOperation = async (body, creatorId, icdvId) => {
  const {
  ...
  const r = await query(
    `INSERT INTO operations (icdv_id, vehicle_id, driver_id, operation_type, scheduled_date, notes, status, created_by)
     VALUES (?,?,?,?,?,?,?,?)`,
    [icdvId, vehicle_id, driver_id, operation_type, scheduled_date, notes, status, creatorId]
  );
  ...
  return getOperationById(r.insertId, icdvId);
};

const getOperations = async ({ page, limit, vehicle_id, driver_id, status, operation_type, search, date_from, date_to }, icdvId = null) => {
  const { limit: l, offset, paginate } = buildPagination(page, limit);
  let where = '1=1'; const params = [];
  if (icdvId !== null)    { where += ' AND op.icdv_id=?';         params.push(icdvId); }
  ...
  const operations = await query(
    `SELECT op.*,
     ...
     WHERE ${where} ORDER BY op.created_at DESC LIMIT ? OFFSET ?`,
    [...params, l, offset]
  );
...
const getOperationById = async (id, icdvId = null) => {
  ...
  if (icdvId !== null && op.icdv_id !== icdvId)
    throw new ApiError(httpStatus.NOT_FOUND, 'Operation not found');
  return op;
};

const updateOperation = async (id, body, updaterId, icdvId = null) => {
  await getOperationById(id, icdvId);
  ...
  if (!fields.length) return getOperationById(id, icdvId);
  ...
  return getOperationById(id, icdvId);
};

const updateOperationStatus = async (id, newStatus, userId, notes = null, icdvId = null) => {
  const op = await getOperationById(id, icdvId);
  const allowed = OPERATION_STATUS_TRANSITIONS[op.status] || [];
  if (!allowed.includes(newStatus))
    throw new ApiError(httpStatus.BAD_REQUEST, `Cannot transition from '${op.status}' to '${newStatus}'`);

  const extraFields = []; const extraParams = [];
  if (newStatus === 'completed') extraFields.push('completed_date=NOW()');
  if (notes) { extraFields.push('notes=?'); extraParams.push(notes); }

  const setClause = [`status=?`, ...extraFields, 'updated_by=?', 'updated_at=NOW()'].join(',');
  await query(`UPDATE operations SET ${setClause} WHERE operation_id=?`,
    [newStatus, ...extraParams, userId, id]);

  if (newStatus === 'completed') {
    await query("UPDATE vehicles SET operational_status='ready', updated_by=?, updated_at=NOW() WHERE vehicle_id=?",
  ...
  if (newStatus === 'cancelled') {
    ...
    if (cnt === 0) {
      await query("UPDATE vehicles SET operational_status='pending', updated_by=?, updated_at=NOW() WHERE vehicle_id=?",
        [userId, op.vehicle_id]);
    }
  }
  return getOperationById(id, icdvId);
...
const deleteOperation = async (id, icdvId = null) => {
  const op = await getOperationById(id, icdvId);
