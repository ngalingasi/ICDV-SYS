...
import { dashboardApi, vesselsApi } from '../../api';
import type { DashboardData, Vessel } from '../../types';
                ...
                <Link key={v.vessel_id} to={`/vessels/${v.vessel_id}`}
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-gray-500 mt-0.5">
                      {v.shipping_line ?? 'No shipping line'} · Arrived {fmtDate(v.arrival_date)}
                    </p>
                    <p className="text-xs text-gray-400 mt-0.5">
                      {v.manifest_count} manifests · {v.vehicle_count} vehicles
                    </p>
                  </div>
                  <span className={`text-xs font-medium capitalize ml-3 ${vesselStatusColors[v.status]}`}>{v.status}</span>
                </Link>
              ))
            )}
          </div>
