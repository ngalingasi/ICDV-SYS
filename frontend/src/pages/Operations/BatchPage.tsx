...
export default function BatchPage() {
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-4 sm:space-y-5">
      ...
      {/* Header row — shrinks gracefully, link wraps below on xs */}
      ...
      <div className="rounded-lg bg-blue-50 dark:bg-blue-500/10 border border-blue-200 dark:border-blue-500/20 px-3 py-2.5 text-xs text-blue-700 dark:text-blue-400 leading-relaxed">
        System assigns vehicles to the current open batch for the vessel (max 20 per batch). A new batch is created automatically only when the current batch becomes full — batches continue across days.
      </div>
          ...
          <Section title="Step 3 — Confirm Batch Assignment">
            <div className="rounded-lg bg-gray-50 dark:bg-gray-800/50 border border-gray-100 dark:border-gray-700 px-3 py-3 text-sm space-y-1">
              ...
              <p className="text-xs text-gray-500 dark:text-gray-400">
                Vehicle will be added to the current open batch for this vessel. A new batch is created only when the current batch reaches 20 vehicles.
              </p>
            </div>
      ...
      {step === 'done' && result && (
        <div className="rounded-xl bg-green-50 dark:bg-green-500/10 border border-green-200 dark:border-green-500/20 p-5 sm:p-6 text-center space-y-3">
          ...
          <div className="inline-block rounded-lg bg-white dark:bg-gray-900 border border-green-200 dark:border-green-500/30 px-4 py-2">
            <p className="font-mono font-bold text-gray-900 dark:text-white text-base sm:text-lg break-all">
              {result.batch_number}
            </p>
            <p className="text-xs text-gray-500 dark:text-gray-400">Batch Number</p>
          </div>

          {/* Buttons: stack on xs, row on xsm+ */}
          <div className="flex flex-col xsm:flex-row gap-2 justify-center items-stretch xsm:items-center">
            <button
              onClick={reset}
            ...
            <Link
              to={`/operations/batches/${result.batch_id}`}
